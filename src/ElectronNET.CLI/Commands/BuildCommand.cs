...
public class BuildCommand : ICommand
{
    ...
    private readonly string _paramAbsoluteOutput = "absolute-path";
    private readonly string _paramDotNetConfig = "dotnet-configuration";
    private readonly string _paramElectronArch = "electron-arch";
    private readonly string _paramElectronParams = "electron-params";
    private readonly string _paramForceNodeInstall = "install-modules";
    ...
    public Task<bool> ExecuteAsync()
    {
        return Task.Run(() =>
        {
            ...
            Console.WriteLine(
                $"Build ASP.NET Core App for {platformInfo.NetCorePublishRid} under {configuration}-Configuration...");
            ...
            if (Directory.Exists(checkForNodeModulesDirPath) == false || parser.Contains(_paramForceNodeInstall) ||
                parser.Contains(_paramPackageJson))
    ...
    private Dictionary<string, string> GetDotNetPublishFlags(SimpleCommandLineParser parser)
    {
        ...
        if (parser.Arguments.ContainsKey(_paramVersion))
        {
            ...
            if (parser.Arguments.Keys.All(key =>
                    !key.StartsWith("p:ProductVersion=") && !key.StartsWith("property:ProductVersion=")))
        ...
        foreach (var parm in parser.Arguments.Keys.Where(key => key.StartsWith("p:") || key.StartsWith("property:")))
        {
            ...
            // normalize the key
            if (key.StartsWith("/property:")) key = key.Replace("/property:", "/p:");
