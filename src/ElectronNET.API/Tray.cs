...
public sealed class Tray
{
    ...
    public event Action OnBalloonShow
    {
        add
        {
            ...
            {
                BridgeConnector.Socket.On("tray-balloon-show-event" + GetHashCode(), () => { _balloonShow(); });
    ...
    public event Action OnBalloonClick
    {
        add
        {
            ...
            {
                BridgeConnector.Socket.On("tray-balloon-click-event" + GetHashCode(), () => { _balloonClick(); });
    ...
    public event Action OnBalloonClosed
    {
        add
        {
            ...
            {
                BridgeConnector.Socket.On("tray-balloon-closed-event" + GetHashCode(), () => { _balloonClosed(); });
    ...
    public async Task Show(string image, MenuItem[] menuItems)
    {
        ...
        {
            var menuItem = _items.GetMenuItem(id);
            menuItem?.Click();
        });
    }
