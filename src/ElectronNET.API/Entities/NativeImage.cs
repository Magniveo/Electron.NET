...
public class NativeImage
{
    ...
    /// </summary>
    public bool IsTemplateImage { get; private set; }
    ...
    {
        if (options is null) options = new CreateFromBitmapOptions();
    ...
    {
        if (options is null) options = new CreateFromBufferOptions();
    ...
    public static NativeImage CreateFromPath(string path)
    {
        ...
        if (Regex.IsMatch(path, "(@.+?x)"))
        {
            ...
            if (dpi == null) throw new Exception($"Invalid scaling factor for '{path}'.");
        ...
        else
        {
            ...
            foreach (var scale in ScaleFactorPairs)
            {
                ...
                if (File.Exists(fileName))
                {
                    ...
                    if (dpi != null) images[dpi.Value] = Image.FromFile(fileName);
                }
            }
        }
    ...
    {
        var images = new Dictionary<float, Image>();
        foreach (var image in _images) images.Add(image.Key, Crop(rect.X, rect.Y, rect.Width, rect.Height, image.Key));
    ...
    {
        var images = new Dictionary<float, Image>();
        foreach (var image in _images) images.Add(image.Key, Resize(options.Width, options.Height, image.Key));
    ...
    {
        if (!_images.ContainsKey(options.ScaleFactor)) return null;

        var image = _images[options.ScaleFactor];
        var mimeType = ImageCodecInfo.GetImageEncoders().FirstOrDefault(x => x.FormatID == image.RawFormat.Guid)
            ?.MimeType;
    ...
    {
        if (!_images.ContainsKey(scaleFactor) || (width is null && height is null)) return null;
    ...
    {
        if (!_images.ContainsKey(scaleFactor)) return null;
        ...
        using (var g = Graphics.FromImage(image))
        {
            ...
            var bmp = new Bitmap(width.Value, height.Value);
            g.DrawImage(bmp, new System.Drawing.Rectangle(0, 0, image.Width, image.Height),
                new System.Drawing.Rectangle(x.Value, y.Value, width.Value, height.Value), GraphicsUnit.Pixel);
    ...
    internal Dictionary<float, string> GetAllScaledImages()
    {
        ...
        try
        {
            foreach (var (scale, image) in _images) dict.Add(scale, Convert.ToBase64String(ImageToBytes(null, scale)));
        }
        catch (Exception ex)
        {
    ...
    {
        if (_images.ContainsKey(scaleFactor)) return _images[scaleFactor];
