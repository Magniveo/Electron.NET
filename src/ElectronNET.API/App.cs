...
public sealed class App
{
    ...
    public string Name
    {
        ...
        get => NameAsync.Result;
        set => BridgeConnector.Socket.Emit("appSetName", value);
    }
    ...
    /// <summary>
    ///     A <see cref="CommandLine" /> object that allows you to read and manipulate the command line arguments that Chromium
    ///     uses.
    /// </summary>
    ...
    public string UserAgentFallback
    {
        ...
        get => UserAgentFallbackAsync.Result;
        set => BridgeConnector.Socket.Emit("appSetUserAgentFallback", value);
    }
            ...
            if (_windowAllClosed == null)
            {
                BridgeConnector.Socket.On("app-window-all-closed" + GetHashCode(), () =>
                {
                    if (!Electron.WindowManager.IsQuitOnWindowAllClosed ||
                        RuntimeInformation.IsOSPlatform(OSPlatform.OSX)) _windowAllClosed();
                });
    ...
    ///     <para />
    ///     See the description of the <see cref="WindowAllClosed" /> event for the differences between the
    ///     <see cref="WillQuit" />
    ...
    public event Action BrowserWindowBlur
    {
        add
        {
            ...
            {
                BridgeConnector.Socket.On("app-browser-window-blur" + GetHashCode(), () => { _browserWindowBlur(); });
    ...
    public event Action BrowserWindowFocus
    {
        add
        {
            ...
            {
                BridgeConnector.Socket.On("app-browser-window-focus" + GetHashCode(), () => { _browserWindowFocus(); });
    ...
    public event Action WebContentsCreated
    {
        add
        {
            ...
            {
                BridgeConnector.Socket.On("app-web-contents-created" + GetHashCode(), () => { _webContentsCreated(); });
    ...
    ///     screen readers, are enabled or disabled. See https://www.chromium.org/developers/design-documents/accessibility for
    ///     more details.
    /// </summary>
    ...
    public event Action<bool> AccessibilitySupportChanged
    {
        add
        {
            ...
            {
                BridgeConnector.Socket.On("app-accessibility-support-changed" + GetHashCode(),
                    state => { _accessibilitySupportChanged((bool)state); });
    ...
    public event Action<string> OpenFile
    {
        add
        {
            if (_openFile == null)
            {
                BridgeConnector.Socket.On("app-open-file" + GetHashCode(), file => { _openFile(file.ToString()); });
    ...
    public event Action<string> OpenUrl
    {
        add
        {
            if (_openUrl == null)
            {
                BridgeConnector.Socket.On("app-open-url" + GetHashCode(), url => { _openUrl(url.ToString()); });
    ...
    /// <summary>
    ///     Try to close all windows. The <see cref="BeforeQuit" /> event will be emitted first. If all windows are
    ///     successfully
    ///     closed, the <see cref="WillQuit" /> event will be emitted and by default the application will terminate. This
    ///     method
    ...
    /// <summary>
    ///     All windows will be closed immediately without asking user and the <see cref="BeforeQuit" /> and
    ///     <see cref="WillQuit" />
    ...
    ///     <para />
    ///     Note that this method does not quit the app when executed, you have to call <see cref="Quit" /> or
    ///     <see cref="Exit" />
    ///     after calling <see cref="Relaunch()" /> to make the app restart.
    ///     <para />
    ///     When <see cref="Relaunch()" /> is called for multiple times, multiple instances will be started after current
    ///     instance
    ///     exited.
    /// </summary>
    ...
    ///     <see cref="RelaunchOptions.Args" /> will be passed as command line arguments instead. When
    ///     <see cref="RelaunchOptions.ExecPath" />
    ...
    ///     that does not exist, an Error is thrown. In that case, the directory should be created with fs.mkdirSync or
    ///     similar.
    ///     <para />
    ///     You can only override paths of a name defined in <see cref="GetPathAsync" />.
    ///     <para />
    ///     By default, web pages' cookies and caches will be stored under the <see cref="PathName.UserData" /> directory. If
    ///     you
    ///     want to change this location, you have to override the <see cref="PathName.UserData" /> path before the
    ///     <see cref="Ready" />
    ...
    ///     <see href="https://www.electronjs.org/docs/api/locales">here</see>.
    ///     <para />
    ...
    ///     <see
    ///         href="https://developer.apple.com/library/ios/documentation/General/Reference/InfoPlistKeyReference/Articles/CoreFoundationKeys.html#//apple_ref/doc/uid/TP40009249-102207-TPXREF115">
    ///         Apple's
    ///         documentation
    ///     </see>
    ///     for details.
    ...
    ///     application as a default protocol handler you
    ///     <see href="https://docs.microsoft.com/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-uap-protocol">
    ///         must
    ///         declare the protocol in your manifest
    ///     </see>
    ///     .
    ///     <para />
    ...
    ///     on the macOS machine. Please refer to
    ///     <see
    ///         href="https://developer.apple.com/library/mac/documentation/Carbon/Reference/LaunchServicesReference/#//apple_ref/c/func/LSCopyDefaultHandlerForURLScheme">
    ///         Apple's
    ///         documentation
    ///     </see>
    ///     for details.
    ...
    ///     <para />
    ///     I.e.This method returns <see langword="true" /> if your process is the primary instance of your application and
    ///     your
    ...
    /// </returns>
    public async Task<bool> RequestSingleInstanceLockAsync(Action<string[], string> newInstanceOpened,
        CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        ...
        using (cancellationToken.Register(() => taskCompletionSource.TrySetCanceled()))
        {
            ...
            BridgeConnector.Socket.On("secondInstance", result =>
            {
                ...
                var args = results.First.ToObject<string[]>();
                var workingDirectory = results.Last.ToObject<string>();
    ...
    ///     eligible for
    ///     <see
    ///         href="https://developer.apple.com/library/ios/documentation/UserExperience/Conceptual/Handoff/HandoffFundamentals/HandoffFundamentals.html">
    ///         Handoff
    ///     </see>
    ///     to another device afterward.
    /// </summary>
    /// <param name="type">
    ///     Uniquely identifies the activity. Maps to
    ///     <see
    ///         href="https://developer.apple.com/library/ios/documentation/Foundation/Reference/NSUserActivity_Class/index.html#//apple_ref/occ/instp/NSUserActivity/activityType">
    ...
    ///     The webpage to load in a browser if no suitable app is installed on the resuming device. The scheme must be http or
    ///     https.
    /// </param>
    ...
    ///         Handoff
    ///     </see>
    ///     user activity as inactive without invalidating it.
    /// </summary>
    ...
    /// <summary>
    ///     Changes the
    ///     <see href="https://msdn.microsoft.com/en-us/library/windows/desktop/dd378459(v=vs.85).aspx">
    ...
    ///     <see href="https://www.electronjs.org/docs/tutorial/desktop-environment-integration#unity-launcher">
    ///         Desktop
    ...
    ///     To work with Electron's autoUpdater on Windows, which uses
    ///     <see href="https://github.com/Squirrel/Squirrel.Windows">Squirrel</see>,
    ...
    /// <summary>
    ///     <see langword="true" /> if Chrome's accessibility support is enabled, <see langword="false" /> otherwise. This API
    ///     will
    ///     return <see langword="true" /> if the use of assistive technologies, such as screen readers, has been detected.
    ///     See <see href="chromium.org/developers/design-documents/accessibility">Chromium's accessibility docs</see> for more
    ///     details.
    /// </summary>
    ...
    /// <summary>
    ///     Manually enables Chrome's accessibility support, allowing to expose accessibility switch to users in application
    ///     settings.
    ///     See <see href="chromium.org/developers/design-documents/accessibility">Chromium's accessibility docs</see> for more
    ///     details.
    ...
    ///     <para />
    ///     Note: Rendering accessibility tree can significantly affect the performance of your app. It should not be enabled
    ///     by default.
    /// </summary>
    /// <param name="enabled">
    ///     Enable or disable
    ///     <see href="https://developers.google.com/web/fundamentals/accessibility/semantics-builtin/the-accessibility-tree">
    ///         accessibility
    ///         tree
    ///     </see>
    ///     rendering.
    ...
    ///     <see
    ///         href="https://developer.apple.com/reference/appkit/nsapplication/1428479-orderfrontstandardaboutpanelwith?language=objc">
    ///         Apple
    ///         docs
    ///     </see>
    ///     for more details. On Linux, values must be set in order to be shown; there are no defaults.
    ///     <para />
    ///     If you do not set credits but still wish to surface them in your app, AppKit will look for a file named
    ///     "Credits.html",
    ///     "Credits.rtf", and "Credits.rtfd", in that order, in the bundle returned by the NSBundle class method main. The
    ///     first file
    ///     found is used, and if none is found, the info area is left blank. See Apple
    ///     <see href="https://developer.apple.com/documentation/appkit/nsaboutpaneloptioncredits?language=objc">documentation</see>
