...
public sealed class WindowManager
{
    ...
    /// <param name="loadUrl">The load URL.</param>
    /// <returns></returns>
    public async Task<BrowserWindow> CreateWindowAsync(BrowserWindowOptions options,
        string loadUrl = "http://localhost")
    {
        ...
        if (loadUrl.ToUpper() == "HTTP://LOCALHOST") loadUrl = $"{loadUrl}:{BridgeSettings.WebPort}";
        ...
        if (options.X == -1 && options.Y == -1)
        {
            ...
            await BridgeConnector.Socket.Emit("createBrowserWindow", JObject.FromObject(options, _jsonSerializer),
                loadUrl);
        }
        else
        {
            ...
            await BridgeConnector.Socket.Emit("createBrowserWindow", JObject.FromObject(options, ownjsonSerializer),
                loadUrl);
        }
    ...
    public async Task<BrowserView> CreateBrowserViewAsync(BrowserViewConstructorOptions options)
    {
        ...
        BridgeConnector.Socket.On("BrowserViewCreated", id =>
        {
            ...
            var browserView = new BrowserView(int.Parse(browserViewId));
