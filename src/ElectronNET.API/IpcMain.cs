...
public sealed class IpcMain
{
    ...
    internal static IpcMain Instance
    {
        get
        {
            if (_ipcMain == null)
                lock (_syncRoot)
                {
                    if (_ipcMain == null) _ipcMain = new IpcMain();
                }
    ...
    {
        var objectArray = ((JArray)args).ToObject<object[]>().ToList();

        for (var index = 0; index < objectArray.Count; index++)
        {
            var item = objectArray[index];
            if (item == null) objectArray.Remove(item);
        }
    ...
    public void Send(BrowserWindow browserWindow, string channel, params object[] data)
    {
        ...
        foreach (var parameterObject in data)
            if (parameterObject.GetType().IsArray ||
                (parameterObject.GetType().IsGenericType && parameterObject is IEnumerable))
                jarrays.Add(JArray.FromObject(parameterObject, _jsonSerializer));
            else if (parameterObject.GetType().IsClass && !parameterObject.GetType().IsPrimitive &&
                     !(parameterObject is string))
                jobjects.Add(JObject.FromObject(parameterObject, _jsonSerializer));
            else if (parameterObject.GetType().IsPrimitive || parameterObject is string) objects.Add(parameterObject);

        if (jobjects.Count > 0 || jarrays.Count > 0)
            BridgeConnector.Socket.Emit("sendToIpcRenderer", JObject.FromObject(browserWindow, _jsonSerializer),
                channel, jarrays.ToArray(), jobjects.ToArray(), objects.ToArray());
        else
            BridgeConnector.Socket.Emit("sendToIpcRenderer", JObject.FromObject(browserWindow, _jsonSerializer),
                channel, data);
    }
    ...
    public void Send(BrowserView browserView, string channel, params object[] data)
    {
        ...
        if (jobjects.Count > 0 || jarrays.Count > 0)
            BridgeConnector.Socket.Emit("sendToIpcRendererBrowserView", browserView.Id, channel, jarrays.ToArray(),
                jobjects.ToArray(), objects.ToArray());
        else
