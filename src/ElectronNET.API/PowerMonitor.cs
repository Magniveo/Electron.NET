...
public sealed class PowerMonitor
{
    ...
    internal static PowerMonitor Instance
    {
        get
        {
            if (_powerMonitor == null)
                lock (_syncRoot)
                {
                    if (_powerMonitor == null) _powerMonitor = new PowerMonitor();
                }
    ...
    public event Action OnLockScreen
    {
        add
        {
            if (_lockScreen == null)
            {
                BridgeConnector.Socket.On("pm-lock-screen", () => { _lockScreen(); });
    ...
    public event Action OnUnLockScreen
    {
        add
        {
            ...
            {
                BridgeConnector.Socket.On("pm-unlock-screen", () => { _unlockScreen(); });
    ...
    public event Action OnSuspend
    {
        add
        {
            if (_suspend == null)
            {
                BridgeConnector.Socket.On("pm-suspend", () => { _suspend(); });
    ...
    public event Action OnResume
    {
        add
        {
            if (_resume == null)
            {
                BridgeConnector.Socket.On("pm-resume", () => { _resume(); });
    ...
    public event Action OnAC
    {
        add
        {
            if (_onAC == null)
            {
                BridgeConnector.Socket.On("pm-on-ac", () => { _onAC(); });
    ...
    public event Action OnBattery
    {
        add
        {
            if (_onBattery == null)
            {
                BridgeConnector.Socket.On("pm-on-battery", () => { _onBattery(); });
    ...
    public event Action OnShutdown
    {
        add
        {
            if (_shutdown == null)
            {
                BridgeConnector.Socket.On("pm-shutdown", () => { _shutdown(); });
