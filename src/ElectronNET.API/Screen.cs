...
public sealed class Screen
{
    ...
    internal static Screen Instance
    {
        get
        {
            if (_screen == null)
                lock (_syncRoot)
                {
                    if (_screen == null) _screen = new Screen();
                }
    ...
    public event Action<Display> OnDisplayAdded
    {
        add
        {
            ...
            {
                BridgeConnector.Socket.On("screen-display-added-event" + GetHashCode(),
                    display => { _onDisplayAdded(((JObject)display).ToObject<Display>()); });
    ...
    public event Action<Display> OnDisplayRemoved
    {
        add
        {
            ...
            {
                BridgeConnector.Socket.On("screen-display-removed-event" + GetHashCode(),
                    display => { _onDisplayRemoved(((JObject)display).ToObject<Display>()); });
