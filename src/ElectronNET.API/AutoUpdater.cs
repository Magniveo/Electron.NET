...
public sealed class AutoUpdater
{
    ...
    public bool AutoDownload
    {
        ...
        set => BridgeConnector.Socket.Emit("autoUpdater-autoDownload-set", value);
    }
    ...
    public bool AutoInstallOnAppQuit
    {
        ...
        set => BridgeConnector.Socket.Emit("autoUpdater-autoInstallOnAppQuit-set", value);
    }

    /// <summary>
    ///     *GitHub provider only.* Whether to allow update to pre-release versions.
    ///     Defaults to "true" if application version contains prerelease components (e.g. "0.12.1-alpha.1", here "alpha" is a
    ///     prerelease component), otherwise "false".
    ...
    public bool AllowPrerelease
    {
        ...
        set => BridgeConnector.Socket.Emit("autoUpdater-allowPrerelease-set", value);
    }
    ...
    public bool FullChangelog
    {
        ...
        set => BridgeConnector.Socket.Emit("autoUpdater-fullChangelog-set", value);
    }
    ...
    public bool AllowDowngrade
    {
        ...
        set => BridgeConnector.Socket.Emit("autoUpdater-allowDowngrade-set", value);
    }
        ...
        get
        {
            return Task.Run(() =>
            {
                ...
                BridgeConnector.Socket.On("autoUpdater-currentVersion-get-reply", result =>
                {
                    ...
                    var version = ((JObject)result).ToObject<SemVer>();
    ...
    public string Channel => ChannelAsync.Result;
        ...
        get
        {
            return Task.Run(() =>
            {
                ...
                BridgeConnector.Socket.On("autoUpdater-requestHeaders-get-reply", headers =>
                {
                    ...
                    var result = ((JObject)headers).ToObject<Dictionary<string, string>>();
    ...
    internal static AutoUpdater Instance
    {
        get
        {
            if (_autoUpdater == null)
                lock (_syncRoot)
                {
                    if (_autoUpdater == null) _autoUpdater = new AutoUpdater();
                }
    ...
    public event Action<string> OnError
    {
        add
        {
            if (_error == null)
            {
                BridgeConnector.Socket.On("autoUpdater-error" + GetHashCode(),
                    message => { _error(message.ToString()); });
    ...
    public event Action<UpdateInfo> OnUpdateAvailable
    {
        add
        {
            ...
            {
                BridgeConnector.Socket.On("autoUpdater-update-available" + GetHashCode(),
                    updateInfo => { _updateAvailable(JObject.Parse(updateInfo.ToString()).ToObject<UpdateInfo>()); });
    ...
    public event Action<UpdateInfo> OnUpdateNotAvailable
    {
        add
        {
            ...
            {
                BridgeConnector.Socket.On("autoUpdater-update-not-available" + GetHashCode(),
                    updateInfo =>
                    {
    ...
    public event Action<ProgressInfo> OnDownloadProgress
    {
        add
        {
            ...
            {
                BridgeConnector.Socket.On("autoUpdater-download-progress" + GetHashCode(),
                    progressInfo =>
                    {
    ...
    public event Action<UpdateInfo> OnUpdateDownloaded
    {
        add
        {
            ...
            {
                BridgeConnector.Socket.On("autoUpdater-update-downloaded" + GetHashCode(),
                    updateInfo => { _updateDownloaded(JObject.Parse(updateInfo.ToString()).ToObject<UpdateInfo>()); });
        ...
        BridgeConnector.Socket.On("autoUpdaterCheckForUpdatesComplete" + guid, updateCheckResult =>
        {
            try
            {
                ...
                taskCompletionSource.SetResult(
                    JObject.Parse(updateCheckResult.ToString()).ToObject<UpdateCheckResult>());
            }
            catch (Exception ex)
            {
        ...
        BridgeConnector.Socket.On("autoUpdaterCheckForUpdatesAndNotifyComplete" + guid, updateCheckResult =>
        {
            try
            {
                ...
                if (updateCheckResult == null)
                    ...
                    taskCompletionSource.SetResult(JObject.Parse(updateCheckResult.ToString())
                        .ToObject<UpdateCheckResult>());
            }
            catch (Exception ex)
            {
                taskCompletionSource.SetException(ex);
