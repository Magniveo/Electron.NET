...
public class WindowsController : Controller
{
    public IActionResult Index()
    {
        ...
        {
            var viewPath = $"http://localhost:{BridgeSettings.WebPort}/windows/demowindow";

            Electron.IpcMain.On("new-window",
                async args => { await Electron.WindowManager.CreateWindowAsync(viewPath); });
    ...
    private async void UpdateReply()
    {
        ...
        var message = $"Size: {size[0]},{size[1]} Position: {position[0]},{position[1]}";
