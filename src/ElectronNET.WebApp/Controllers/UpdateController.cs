    ...
    public IActionResult Index()
    {
        if (HybridSupport.IsElectronActive)
        {
            ...
            Electron.AutoUpdater.OnCheckingForUpdate +=
                async () => await Electron.Dialog.ShowMessageBoxAsync("Checking for Update");
            Electron.AutoUpdater.OnUpdateNotAvailable += async info =>
                await Electron.Dialog.ShowMessageBoxAsync("Update not available");
            ...
            Electron.IpcMain.On("auto-update", async args =>
            {
                ...
                var information = $"Current version: {currentVersion} - available version: {availableVersion}";
